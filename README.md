...
The same production build is mirrored at `https://making-small-memories-concepts.xenvya.workers.dev` in the Xenvya account for review. That hostname displays a compact concept-preview notice linking to the live customer website; the notice stays hidden on the customer domain. The mirror deployment uses `wrangler.xenvya.jsonc` and has no custom-domain routes. Run `npm run deploy:concept` after the customer-domain deployment whenever both public addresses should remain synchronized.
