...
- `npm test`: 19 local browser scenarios passed, covering the selected homepage and privacy route, five legacy redirects, brand lockup, approved external links, and booking/payment walkthroughs.
...
- Real browser checks: mobile menu, Escape, skip link, focus visibility, FAQs, service detail disclosures, consultation draft feedback, telephone/email/Facebook/VA.org links and all internal links/anchors.
...
Detailed machine-readable results are stored in `release-audit.json`. The final local unthrottled audit measured approximately 366 KB transferred for the homepage and 39 KB for privacy, with observed CLS below 0.009. These are local-agent browser observations, not Lighthouse scores or field Core Web Vitals data. No mobile-network performance claim is implied.
